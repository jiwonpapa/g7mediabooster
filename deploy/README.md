...
선택형 공개 썸네일 listener는 같은 `g7mb-api` 프로세스의 `127.0.0.1:8089`에서 별도 router로
실행됩니다. 기본 비활성이며 [공개 썸네일 전달](../docs/PUBLIC_THUMBNAIL_DELIVERY.md)의
path-only Nginx 설정과 실제 ingress gate 전에는 공식 지원 기능으로 게시하지 않습니다.
...
`output/releases/jiwonpapa-g7mediabooster-0.4.3.zip`과 서버 수동 배치용 `.tar.gz`, 각각의
`.sha256`을 생성합니다. G7 관리자 **모듈 관리 → 파일에서 설치**에는 ZIP을 사용합니다.
