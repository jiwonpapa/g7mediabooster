...
`0001`~`0005` 적용을 전제로 공식 게시합니다. 0.4.1의 관리자 워터마크 자산 선택·rollback과
...
`output/releases/jiwonpapa-g7mediabooster-0.4.1.tar.gz`와 같은 이름의 `.sha256`을 생성합니다.
압축본은 Git commit의 module subtree에서 재현 가능하게 만들며 `vendor`, `node_modules`, tests,
PHPUnit cache를 포함하지 않습니다. 압축 내부 `module.json` 버전과 공식 기능 manifest 버전이
다르면 생성 자체를 거부합니다.
