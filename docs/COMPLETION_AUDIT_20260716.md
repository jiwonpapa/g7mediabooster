...
> 2026-07-16 snapshot입니다. 이후 R2 실계정 검증은 통과했고 공식 G7 7.0.6 stock capability는
> 차단 상태로 재판정됐습니다. 현재 판정은 `deploy/official-features-v1.json`을 따릅니다.
...
| 2 S3 호환 업로드 | PARTIAL | pinned MinIO single/multipart/client 재생성 재개/abort/HEAD/GET/PUT/Delete, profile-bound 외부 credential·browser OPTIONS/PUT CORS 하네스, Lightsail management fail-closed | R2·Lightsail 각각 실계정 protocol·browser CORS PASS |
...
| 17 운영 기능 | PASS | lifecycle/quota/orphan audit-prune/backup-restore/API admission/metrics/tombstone purge, payload SHA-256·native runtime·systemd target·setup·MinIO doctor를 잇는 Linux 통합 설치 gate | production Prometheus·alert 연결 |
...
현재 관리자 설치판 0.4.3은 module commit `7df2432` 기준 ZIP 153,244 bytes,
SHA-256 `1137edfb…ecac`, tar.gz 126,297 bytes, SHA-256 `160c980a…0da1`로 재현했습니다.
공개 G7 `fcaacad` patch clean apply·MySQL 8.4 회귀 115/283과 현재 G7의 실제
`ZipInstallHelper` identifier·version·capability manifest를 모두 통과했습니다.

## 설치·비밀값 경로 보강

`g7mbctl setup` 대화형 CUI와 비대화형 file-input 모드를 추가했습니다. R2 Account ID endpoint
파생, 공급자 profile, R2/AWS bucket/CORS bootstrap, Lightsail management fail-closed,
bounded single/multipart canary, 설정 충돌 사전검사,
...
실 provider 하네스는 exact G7 origin의 browser OPTIONS/PUT CORS·ETag 노출, protocol 삭제 뒤
`HEAD NotFound`와 실제 SQLite lifecycle의 Ready 사용자
