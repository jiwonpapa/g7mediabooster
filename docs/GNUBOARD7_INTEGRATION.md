...
- 상태: form 자동 연결·Ready attachment bridge·private viewer·삭제 보존 대조 구현, 격리 browser 전송/create/update/private thumbnail과 권한·삭제/복원·보존 lease G7 DB gate PASS, upstream 정식 반영·실 provider 삭제는 보류
...
모듈 0.3.1은 원본 파일명을 G7 내부에만 저장하고 Rust에는 기존 좁은 upload intent만 보냅니다.
...
삭제/복원·보존 lease는 G7 host gate에서 통과했습니다. 실브라우저 거부 매트릭스와 실 provider
보존 만료 삭제는 운영 게이트로 유지합니다.
...
준비했습니다. 별도 깨끗한 worktree에서 patch 5개 clean apply, 계약 검사 28/28, 첨부 전체 PHP
...
upstream 미반영 배포와 실브라우저/실 provider 종단은 해당 게이트 전 지원으로 게시하지 않습니다.
