...
- 상태: form 자동 연결·Ready attachment bridge·private viewer·삭제 보존 대조 구현, 격리 설치·설정과 MinIO 기반 실제 전송·create/update·private thumbnail PASS, upstream·권한·삭제/복원은 보류
...
모듈은 이 여섯 계약이 없으면 manifest와 runtime reflection 검사에서 fail-closed 합니다.
실제 브라우저 single PUT·2-part multipart, Ready attachment 2개 생성·표시, 게시글 수정 후 유지와
private thumbnail 302→MinIO 200은 통과했습니다. 타 사용자·비밀글·삭제글 권한과 삭제/복원은
미완료 게이트로 유지합니다.
...
준비했습니다. 별도 깨끗한 worktree에서 patch 4개 clean apply, 계약 검사 23/23, 핵심 PHP
48/48와 신규 overlay/count 회귀를 확인했으며 전체 board suite는 기존 기준선과 동일한 80개 실패로
신규 회귀가 없었습니다. module 설치·설정·user/admin form과 실제 저장소 E2E도 통과했고 실제
G7 사용자 작업 트리는 수정하지 않았습니다. patch 적용 배포에는 통과 범위만 공식 게시하며,
upstream 미반영 배포와 권한·삭제/복원은 해당 게이트 전 지원으로 게시하지 않습니다.
