...
- 상태: control API/programmatic uploader/private owner delivery implemented, board attachment integration pending
...
이 세 계약이 없으므로 현재 완료 표시는 `G7 제어 API와 programmatic uploader`까지입니다. `attachment_ids` 연결,
...
준비했습니다. patch 적용본은 계약 검사 10/10 PASS, 현재 실제 G7은 10개 누락 FAIL입니다.
upstream 반영과 G7 회귀·브라우저 smoke가 끝난 뒤에만 이 모듈의 attachment bridge를 등록합니다.
