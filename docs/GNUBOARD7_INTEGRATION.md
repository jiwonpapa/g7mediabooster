...
- 상태: form 자동 연결·Ready attachment bridge·private viewer·삭제 보존 대조 구현, 격리 설치·설정·user/admin form disabled smoke PASS, 실제 전송·create/update 전 공식 지원 보류
...
준비했습니다. 별도 깨끗한 worktree에서 patch 2개 clean apply, 계약 검사 21/21, 핵심 PHP
48/48, core overlay 2/2를 확인했으며 전체 board suite는 기존 기준선과 동일한 80개 실패로
신규 회귀가 없었습니다. module 설치·설정·user/admin form disabled smoke도 통과했고 실제
G7 사용자 작업 트리는 수정하지 않았습니다. upstream 반영과 실제 저장소 전송·create/update·
권한·삭제/복원 smoke가 끝난 뒤에만 attachment bridge를 운영 활성화하고 공식 지원으로 게시합니다.
