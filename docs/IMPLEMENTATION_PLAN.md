...
5. 썸네일은 object storage/CDN에 불변 파일로 저장합니다. 메모리에는 작은 manifest만
   ...
   적용안은 적중률이 더 좋은 **W-TinyLFU admission + LRU eviction**입니다.
...
- lazy 생성은 관리자 allowlist preset만 허용하며 임의 `width/quality` query를 금지합니다.
- 생성 결과는 object storage에 영구 저장하고 CDN에
  `Cache-Control: public, max-age=31536000, immutable`로 제공합니다.
- 수정은 overwrite하지 않고 `source_rev` 또는 `preset_rev`를 바꿉니다.
- Rust 메모리는 thumbnail bytes의 진실 원천이 아닙니다. manifest만 byte-weighted
  W-TinyLFU/LRU + TTL로 제한하고 같은 key 동시 생성은 singleflight로 합칩니다.
...
- JPEG/PNG/WebP/GIF/AVIF/HEIC와 MP4 실제 fixture round-trip
...
- 100개 batch의 bounded 스케줄링 계약과 로컬 실제 JPEG worker/만료 lease 복구/RSS 게이트는
  통과. 실 S3/R2에서 multipart 중단, API/worker 강제 종료와 중복 게시 0은 별도 운영 게이트
...
- thumbnail URL revision, CDN header, LRU/TinyLFU eviction, singleflight 검증
