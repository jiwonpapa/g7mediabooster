...
worktree에서 upstream patch를 `sirsoft-board` 1.2.0으로 재배치했습니다. patch 적용과
핵심 회귀는 통과했으며 실제 설치·브라우저 검증은 아래 순서로 계속합니다.

1. `scripts/verify-gnuboard7-media-contract.sh` 21/21 — PASS
...
4. module `cargo xtask g7-adapter`
5. module 0.3.0 설치·세 migration 실행 및 rollback rehearsal
6. 100개 전송 후 Ready attachment ID가 게시글 create/update에 전건 연결되는지 확인
7. 타 사용자 ID, 비밀글, 삭제글, 권한 없는 guest의 master/poster 요청 차단 확인
8. soft-delete 후 복원 취소, 보존 만료 삭제 예약, 요청 시작 뒤 복원 차단을 실제 schedule로 확인
