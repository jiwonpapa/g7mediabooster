...
- 실제 G7 HTTPS origin의 browser `OPTIONS` preflight와 PUT 응답에서 exact
  `Access-Control-Allow-Origin`, PUT 허용, `ETag` 노출 확인
...
- CORS·byte·inventory 검증이 중간 실패해도 protocol run의 알려진 raw/derivative key를 모두
  idempotent DELETE하고 정리 실패까지 최종 오류에 포함

bucket 생성·IAM 변경·CORS 변경은 하지 않지만 이미 설정된 browser CORS가 실제 응답되는지는
검증합니다. object key는
...
설치 경로의 `g7mbctl storage bootstrap`은 별도로 구현됐습니다. R2/AWS S3는 권한이 있을 때
bucket을 생성하고 기존 CORS를 보존한 채 MediaBooster 관리 규칙을 병합합니다. Lightsail은
bucket access key가 버킷 생성·Lightsail `UpdateBucket` CORS 권한을 제공하지 않으므로 두
요청을 네트워크 전에 거부하고 사전 설정된 버킷을 `--skip-cors`로 검사합니다. 이어서
...
```bash
# R2: 설치 CUI에서 --defer-storage를 선택했다면 bucket/CORS와 bounded canary부터 완료
...
# Lightsail: 콘솔/API에서 bucket과 exact-origin CORS를 먼저 만든 뒤 검사만 수행
sudo g7mbctl storage bootstrap \
  --config /etc/g7mediabooster/g7mb.toml \
  --skip-cors
sudo g7mbctl storage doctor --config /etc/g7mediabooster/g7mb.toml
...
`multipart_reconnect=true`, `browser_cors=1`, `object_count=0`, `user_delete=1`, `retention_expired=1`,
`tombstones=2`만 남습니다.
...
- Lightsail에서 하네스 실행 전 bucket과 browser CORS를 Lightsail API/콘솔로 설정
