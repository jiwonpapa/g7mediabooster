...
현재 실제 G7 작업 트리는 `sirsoft-board` 버전 파일을 포함한 사용자 변경과 기준 commit이
달라 upstream patch를 바로 적용하지 않습니다. 별도 깨끗한 브랜치/worktree에서 현재 변경을
보존한 채 patch를 재배치하고 다음 순서로 검증합니다.

1. `scripts/verify-gnuboard7-media-contract.sh` 14/14
2. upstream README의 board PHP 회귀 4개와 module `cargo xtask g7-adapter`
3. module 0.2.0 설치·두 migration 실행 및 rollback rehearsal
4. 100개 전송 후 Ready attachment ID가 게시글 create/update에 전건 연결되는지 확인
5. 타 사용자 ID, 비밀글, 삭제글, 권한 없는 guest의 master/poster 요청 차단 확인
6. soft-delete 보존 만료 reconciler 구현 전 native attachment 삭제는 공식 지원에서 제외

여기까지 통과하기 전 배포 설명에는 G7 자동 form 연동·게시물 첨부 표시를 게시하지 않습니다.
