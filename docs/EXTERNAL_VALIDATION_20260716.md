...
알려진 key는 idempotent DELETE로 정리합니다.
...
직렬 실행합니다. 성공 출력에는 bucket이나 key 대신 `object_count=0`, `user_delete=1`,
`retention_expired=1`, `tombstones=2`만 남습니다.
...
현재 G7 개발 checkout `c275b41b`은 정적 계약 28/28과 PHP·JSON 문법 검사를 통과했습니다.
다만 media contract 관련 경로가 다른 사용자 작업과 함께 아직 uncommitted 상태이므로 정식
upstream 기준으로 승격하지 않습니다. G7MediaBooster 검증은 checkout을 변경하지 않습니다.
분리된 임시 MySQL 8.4로 첨부·권한 58 tests/90 assertions, 전체 layout extension 57/193,
첨부 수 동기화 1/2를 현재 checkout에서 다시 통과했고 일회용 DB와 `.env.testing`은 제거했습니다.
기존 별도 clean worktree에서는 격리 설치와 MinIO 기반 실제 전송·create/update·private
thumbnail 전달까지 통과했습니다.

1. `scripts/verify-gnuboard7-media-contract.sh` 28/28 — PASS
2. 현재 checkout board 첨부·권한 58 tests/90 assertions — PASS
3. 전체 board suite 기준선 비교 — patch 전 80 failed/1102 passed, patch 후 80 failed/1108 passed, 신규 실패 0
4. module 0.4.1 `cargo xtask g7-adapter` — PASS: PHP 57/155, TS 21, typecheck/build
...
12. 보존 만료 command→실 provider 객체 삭제 확인 — 하네스 구현 완료, credential 실행 대기
