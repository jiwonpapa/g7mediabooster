...
`auto`로 고정합니다. Cloudflare의 별도 Account API token value나 클라이언트 인증서는 받지
않습니다.
...
Lightsail bucket access key는 기존 단일 버킷 전용이므로 기본값이 `CreateBucket=false`입니다.
범용 S3도 공급자 관리 API 차이 때문에 기본값은 기존 버킷 검사입니다.
