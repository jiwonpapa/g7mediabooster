...
```text
large-multipart-smoke PASS bytes=5368709120 parts=160 api_rss_start_kib=33808 api_rss_peak_kib=34736 api_rss_delta_kib=928 direct_body=1 quarantined=1
```

Rust API가 batch intent와 160개 part 서명, complete만 처리했습니다. 각 32MiB 본문은
presigned URL로 MinIO에 직접 전송했고 complete 시 HEAD 길이를 확인한 뒤 upload가
`Quarantined`로 진입했습니다. API RSS 증가 상한은 32MiB이며 실측 증가는 928KiB였습니다.
...
full-stack-smoke PASS single_put=1 multipart_parts=2 ready=2 derivatives=4 exif_removed=1
```
