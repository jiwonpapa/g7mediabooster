...
## 제7조. 계약과 상태는 변경 전에 증명한다

1. OpenAPI가 HTTP 계약의 단일 기준입니다.
2. 상태 전이는 명시적 state machine과 compare-and-set으로만 구현합니다.
3. 새 endpoint, 오류 code, 상태, preset 변경은 G5/G7 contract test와 함께 제출합니다.
4. backward-incompatible 변경은 versioned endpoint 또는 migration 기간을 가집니다.
5. provider 구현만 바꾸고 consumer 어댑터를 추측으로 남기지 않습니다.
