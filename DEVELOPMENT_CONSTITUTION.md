...
8. Bash와 Python의 파일별 상한은 현재 통과값에서 자동으로 늘릴 수 없습니다. Bash 정본을
   Python으로 이관할 때는 같은 변경에서 Bash 감소량이 Python 증가량보다 크고 두 언어 합계가
   감소해야 합니다. 새로운 외부 사용자 계약 gate는 사용자 승인, 중복 조사, ADR에 기록된
   최소 LOC와 대안 검토가 있을 때만 예외적으로 늘릴 수 있습니다. 변경 직후 언어별·합계 상한을
   새 통과값으로 다시 고정합니다.
...
## 제12조. 릴리스 이력과 버전은 사용자 계약이다

1. 독립 배포되는 제품은 각각 Keep a Changelog 1.1.0 형식의 `CHANGELOG.md`를 가집니다.
2. 제품 버전은 Semantic Versioning 2.0.0을 따르며 공개 API와 호환성 영향을 기준으로 올립니다.
3. 모든 주목할 변경은 개발 중 `[Unreleased]`에 기록하고 릴리스 때 ISO 날짜가 있는 버전으로
   이동합니다. 커밋 로그 자동 나열은 변경 이력을 대신할 수 없습니다.
4. 배포한 버전의 내용은 바꾸지 않으며 수정은 반드시 새 버전으로 배포합니다.
5. manifest·tag·CHANGELOG drift와 허용되지 않은 변경 분류는 CI와 tag workflow에서 차단합니다.
6. GitHub Release 노트는 CHANGELOG의 해당 버전 섹션을 사용합니다.
