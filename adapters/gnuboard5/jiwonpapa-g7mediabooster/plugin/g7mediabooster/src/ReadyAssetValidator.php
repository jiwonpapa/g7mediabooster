...
final class ReadyAssetValidator
{
    ...
    {
        $uploadId = $this->uuid($session['upload_id'] ?? null);
        if ($this->uuid($status['upload_id'] ?? null) !== $uploadId
        ...
        $order = filter_var($session['attachment_order'] ?? null, FILTER_VALIDATE_INT);
        ...
        if (! isset($byVariant['master'], $byVariant['thumbnail'])
            || $byVariant['master']['preset_id'] !== $byVariant['thumbnail']['preset_id']
        ) {
        ...
        return [
            ...
            'stored_filename' => 'g7mb-'.$uploadId.'.'.$extension,
            'mime_type' => $byVariant['master']['content_type'],
            'size' => $byVariant['master']['byte_len'],
            'thumbnail_size' => $byVariant['thumbnail']['byte_len'],
            'preset_id' => $byVariant['master']['preset_id'],
            'attachment_order' => $order,
            'image_type' => $kind === 'image' ? 2 : 0,
        ];
    }
    ...
    private function normalizedFilename(mixed $value, string $extension): string
    {
        ...
        $stem = trim(pathinfo($value, PATHINFO_FILENAME), " .\t\n\r\0\x0B");
        if ($stem === '') {
            $stem = 'media';
        }
        $suffix = '.'.$extension;
